...
from src.domain.accounting.journal_entry import JournalEntryProcessor, journal_processor
...
class ManagementAccountingAnalyzer:
    ...
    def analyze_product_profitability(
        self, product_id: str, period_days: int = 30
        ...
        try:
            ...
            # 追加情報（簡易版）
            sales_volume = (
                int(product_sales / 150) if product_sales > 0 else 0
            )  # 仮の平均単価で計算
    ...
    def _get_product_sales(
        self, product_id: str, start_date: date, end_date: date
    ) -> float:
        """商品別売上を取得"""
        # 簡易的な実装：売上高から推定
        sales_revenue = self.journal_processor.get_account_balance(
    ...
    def _get_product_costs(
        ...
        """商品別原価を取得"""
        # 簡易的な実装：仕入高から推定
        cost_of_goods = self.journal_processor.get_account_balance(
            "5001", start_date, end_date
        )
    ...
    def calculate_inventory_turnover(self, product_id: str) -> InventoryEfficiency:
        ...
        try:
            ...
            # 在庫回転率計算
            turnover_ratio = (
                annual_cogs / avg_inventory_value if avg_inventory_value > 0 else 0
            )
    ...
    def _get_annual_cogs(self, product_id: str) -> float:
        """年間売上原価を取得"""
        # 年間仕入高を取得
        annual_cost = self.journal_processor.get_account_balance(
            "5001", None, date.today()
        )
    ...
    def analyze_period_profitability(
        ...
        try:
            # 売上高を取得
            sales_revenue = abs(
                self.journal_processor.get_account_balance("4001", start_date, end_date)
            )

            # 売上原価を取得
            cost_of_goods = abs(
                self.journal_processor.get_account_balance("5001", start_date, end_date)
            )
            ...
            operating_expenses = abs(
                self.journal_processor.get_account_balance("6001", start_date, end_date)
            )

            # 営業利益計算
            operating_profit = gross_profit - operating_expenses
            operating_margin = (
                operating_profit / sales_revenue if sales_revenue > 0 else 0
            )
            ...
            return {
                ...
                "analysis": {
                    ...
                    "cost_control": "適切"
                    if operating_expenses < sales_revenue * 0.3
                    else "見直し必要",
                },
            }
    ...
    def generate_profitability_report(
        self, product_ids: Optional[List[str]] = None, period_days: int = 30
    ) -> Dict[str, Any]:
        """収益性レポートを生成"""
        ...
        try:
            ...
            if product_ids:
                for product_id in product_ids:
                    profitability = self.analyze_product_profitability(
            ...
            if report["products"]:
                ...
                report["summary"] = {
                    ...
                    "overall_gross_margin": total_profit / total_revenue
                    if total_revenue > 0
                    else 0,
                    "profitable_products": sum(
                        1 for p in report["products"] if p["gross_profit"] > 0
                    ),
                    "unprofitable_products": sum(
                        1 for p in report["products"] if p["gross_profit"] <= 0
                    ),
                }
    ...
    def analyze_trend(self, periods: int = 6) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(periods):
                ...
                # 期間別収益性分析
                period_analysis = self.analyze_period_profitability(
            ...
            if len(trends) >= 2:
                revenue_trend = self._calculate_trend_direction(
                    [t["revenue"] for t in trends]
                )
                profit_trend = self._calculate_trend_direction(
                    [t["profit"] for t in trends]
                )
                margin_trend = self._calculate_trend_direction(
                    [t["margin"] for t in trends]
                )
            else:
        ...
        try:
            ...
            return {
                ...
                "kpi_summary": {
                    ...
                    "operating_margin": current_profitability.get(
                        "operating_margin", 0
                    ),
                    "inventory_turnover": sum(
                        e["inventory_turnover_ratio"] for e in inventory_efficiency
                    )
                    / len(inventory_efficiency)
                    if inventory_efficiency
                    else 0,
                    "profitable_products_ratio": profitability_report.get(
                        "summary", {}
                    ).get("profitable_products", 0)
                    / max(
                        profitability_report.get("summary", {}).get(
                            "total_products", 1
                        ),
                        1,
                    ),
