        ...
        for product in SAMPLE_PRODUCTS:
            if product.is_available():
                ...
                product_data.update({
                    ...
                    "popularity_score": product.sales_count / max(sum(p.sales_count for p in SAMPLE_PRODUCTS), 1)
                })
...
async def get_categories():
    ...
    try:
        ...
        for category_info in PRODUCT_CATEGORIES.values():
            # カテゴリの商品数と利用可能数を計算
            category_products = [p for p in SAMPLE_PRODUCTS if p.category == category_info.category]
...
async def get_chat_history(customer_id: str, limit: int = 10):
    """顧客のチャット履歴を取得"""
    try:
        history = await conversation_service.get_conversation_history(customer_id, limit)
        ...
        return [
            {
                ...
                "last_message": conv["summary"].split("最新発言: ")[-1] if "最新発言:" in conv["summary"] else None
            }
            for conv in history
        ]
...
async def get_personalized_recommendations(customer_id: str):
    """パーソナライズされた商品推奨を取得"""
    try:
        recommendations = await customer_agent.generate_personalized_recommendations(customer_id)
