...
class KPI_Visualizer:
    ...
    def get_latest_run_id(self) -> str:
        """データベースから最新のrun_idを取得"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute(
                "SELECT DISTINCT run_id FROM benchmarks ORDER BY run_id DESC LIMIT 1"
            )
            ...
            print(f"最新run_id取得エラー: {e}")
            return ""

    def get_best_run_id_for_visualization(self) -> str:
        """データ量が十分で最新のrun_idを取得"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            if results:
                ...
                for run_id, count in results:
                    if count >= 50:
                        print(f"データ量充足: {run_id} ({count}件)")
                        return run_id

                # 50件未満でも最新のものを返す
                best_run_id = results[0][0]
                print(f"データ量不足だが最新を使用: {best_run_id} ({results[0][1]}件)")
                return best_run_id

            return ""
        except Exception as e:
            print(f"最適run_id取得エラー: {e}")
            return ""

    def get_continuous_simulation_run_ids(self) -> List[str]:
        """continuous_simulationで始まる全てのrun_idを取得"""
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            run_ids = [result[0] for result in results]
            print(f"連続シミュレーションrun_id取得: {len(run_ids)}件")
            return run_ids

        except Exception as e:
            print(f"連続シミュレーションrun_id取得エラー: {e}")
            return []

    def get_kpi_history_from_db(self) -> pd.DataFrame:
        """データベースからKPI履歴を取得（最新の1runのみ）"""
        try:
            # 最新の1run_idのみを取得
            latest_run_id = self.get_latest_run_id()
            if not latest_run_id:
                print("最新のrun_idが見つかりません")
                return pd.DataFrame()

            print(f"最新の1runのみ使用: {latest_run_id}")
            ...
            ORDER BY step ASC
            """
            df = pd.read_sql_query(query, conn, params=(latest_run_id,))
            conn.close()

            if df.empty:
                print(f"run_id {latest_run_id} のデータが見つかりません")
                return pd.DataFrame()

            print(f"最新runデータ取得完了: {len(df)}件")

            # データポイントを間引く（最大50ポイントに制限）
            max_points = 50
            ...
            # データポイントを適切な間隔で配置（1ポイントあたり10分間隔）
            df["timestamp"] = [
                base_time + pd.Timedelta(minutes=i * 10) for i in range(len(df))
            ]
    ...
    def get_cumulative_kpi_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            FROM benchmarks
            WHERE run_id LIKE '%test_run%' OR run_id LIKE '%test_case_a%'
            """
    ...
    def create_time_series_plots(self, df: pd.DataFrame) -> Dict[str, plt.Figure]:
        ...
        # Pricing Accuracy and Action Correctness Comparison
        metrics_to_plot = [
            ("pricing_accuracy", "Pricing Accuracy", "blue"),
            ("action_correctness", "Action Correctness", "orange"),
        ]
        ...
        for col in performance_cols:
            if col in df.columns:
                ...
                if col == "stockout_rate":
                    ...
                    performance_labels.append("Inventory Management Efficiency")
                else:
                    score = latest_val * 100 if latest_val <= 1 else latest_val
                    label_map = {
                        "pricing_accuracy": "Pricing Management Efficiency",
                        "action_correctness": "Operational Accuracy",
    ...
    def create_cumulative_kpi_chart(
        ...
        # Total Profit - Noneチェックを追加
        total_profit = cumulative_data.get("total_profit", 0)
        ...
        # Average Stockout Rate - Noneチェックを追加
        avg_stockout_rate = cumulative_data.get("average_stockout_rate", 0)
        if avg_stockout_rate is None:
            avg_stockout_rate = 0.0
        avg_stockout = avg_stockout_rate * 100
